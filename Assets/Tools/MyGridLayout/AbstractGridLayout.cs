...
namespace Tools.MyGridLayout
{
    ...
    public abstract class AbstractGridLayout : MonoBehaviour
    {
        ...
        protected void Align(int rows, int columns, Vector2 spacing, Vector2 paddingVertical, Vector2 paddingHorizontal,
            bool fromBottomToTop = false)
        {
            ChildCount = transform.childCount;
            if (ChildCount == 0) return;
            var exit = false;
            CellWidth = 1 / (float)columns - spacing.x + spacing.x / columns - paddingHorizontal.x / columns -
                        paddingHorizontal.y / columns;
            CellHeight = 1 / (float)rows - spacing.y + spacing.y / rows - paddingVertical.x / rows -
                         paddingVertical.y / rows;
            ...
            if (fromBottomToTop)
                ...
                for (var i = 0; i < rows; i++)
                {
                    for (var j = 0; j < columns; j++)
                    {
                        ...
                        var childRect = transform.GetChild(i * columns + j).GetComponent<RectTransform>();
                        childRect.anchorMax = new Vector2(CellWidth * (j + 1) + spacing.x * j + paddingHorizontal.x,
                            1 - (CellHeight * i + spacing.y * i + paddingVertical.y));
                        childRect.anchorMin = new Vector2(CellWidth * j + spacing.x * j + paddingHorizontal.x,
                            1 - (CellHeight * (i + 1) + spacing.y * i + paddingVertical.y));
                        childRect.offsetMax = Vector2.zero;
                        childRect.offsetMin = Vector2.zero;
                    }
